...
function removeEnds(theInput){
  if(theInput.length < 2){
    return '';
  }

  return theInput.substring(theInput.length,-1).substring(1,theInput.length);
...
function formatWithPadding(inputNumber,padChar,padLength){
  if(padLength > 0){
    return inputNumber.toString().padStart(padLength,padChar)
  }else{
    return inputNumber;
  }
}
